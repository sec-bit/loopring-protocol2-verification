...
Parameter oracle_totalSupply: ERC20StateMap.t -> address -> address -> (ERC20StateMap.t * list Event).
Parameter oracle_balanceOf: ERC20StateMap.t -> address -> address -> address -> (ERC20StateMap.t * list Event).
Parameter oracle_allowance: ERC20StateMap.t -> address -> address -> address -> address -> (ERC20StateMap.t * list Event).
Parameter oracle_transfer: ERC20StateMap.t -> address -> address -> address -> uint -> (ERC20StateMap.t * list Event).
Parameter oracle_transferFrom: ERC20StateMap.t -> address -> address -> address -> address -> uint -> (ERC20StateMap.t * list Event).
Parameter oracle_approve: ERC20StateMap.t -> address -> address -> address -> uint -> (ERC20StateMap.t * list Event).
...
Definition func_totalSupply
  ...
  | (st', evts') => ((wst_update_erc20s wst st'), evts')
  end.
